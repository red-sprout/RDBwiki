...
등장 위치: `Button`, `DbmsBadge`, `DbmsSectionFilter`  
역할: 조건부 className을 합칩니다.  
언어 자체 기능인지 여부: 아닙니다.  
프로젝트에서 쓰인 이유: active/variant 상태별 스타일을 다룹니다.  
...
역할: `## MySQL`, `## PostgreSQL`, `## Oracle`을 필터 기준으로 삼습니다.  
언어 자체 기능인지 여부: 아닙니다.  
프로젝트에서 쓰인 이유: 비교 문서에서 특정 DBMS만 볼 수 있게 합니다.  
초심자가 주의할 점: heading이 정확히 맞지 않으면 필터에 잡히지 않습니다.
