...
- DBMS 필터가 안 보입니다: Markdown에 `## MySQL`, `## PostgreSQL`, `## Oracle`로 시작하는 h2 heading이 있는지 확인합니다.
- DBMS를 선택했는데 SQL 예시가 모두 보입니다: 코드펜스를 `mysql`, `postgresql`, `oracle`로 명시하거나 `lib/dbms-filter.ts`의 `detectSqlDialect()` 추론 키워드를 확인합니다.
