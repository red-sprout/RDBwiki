...
`SqlCodeBlock`, `app/docs/[[...slug]]/page.tsx`, `MarkdownEditor`
...
SQL 코드 블록은 `SqlCodeBlock`으로 보내고, 나머지 code는 `InlineCode`로 렌더링합니다.
...
```tsx
if (className?.includes("language-sql") || className?.includes("language-postgresql")) {
```

### 수정 시 영향받는 파일

Markdown 코드 블록 렌더링이 바뀝니다.

### 이 파일을 이해한 뒤 알아야 하는 것

Markdown 파싱과 실제 스타일 적용은 다른 층입니다. 스타일은 `app/globals.css`에 있습니다.
...
이 tokenizer는 SQL 실행이나 문법 검증을 하지 않습니다.
